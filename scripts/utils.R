...
#' Especificación del lienzo estándar de la casa
#'
#' Se mantiene el argumento `orientation` solo por compatibilidad con scripts
#' antiguos, pero el repositorio ya no genera variantes apaisadas.
house_spec <- function(orientation = "portrait") {
  list(
...
#' Aplica el logo con la configuración estándar de la casa
house_apply_logo <- function(plot, orientation = "portrait", ...) {
  add_logo(plot, ...)
}

#' Conserva la ruta histórica de salida
house_out_path <- function(portrait_path, orientation = "portrait") {
  portrait_path
}
...
#'
#' @param orientation argumento retenido solo por compatibilidad
theme_quantificador <- function(orientation = "portrait") {
  base_margin <- margin(6, 36, 6, 16)
  theme_classic() +
    theme(
